...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double p = 1. / (1+exp(-2.*_J*_beta*(_particle(this->pbc(i-1)).getspin()+_particle(this->pbc(i+1)).getspin())));
    if(x < p) _particle(i).setspin(1); else _particle(i).setspin(-1);
    _naccepted++;
  } else {           // M(RT)^2
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    sumv.zeros();
    for (int i=0; i<_npart; i++){
      int x = int(_rnd.Rannyu(0,3));
      int y = int(_rnd.Rannyu(0,2))*2-1;
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) );
        if (dr < _halfside.min() && _measure_gofr) _measurement(_index_gofr + int(dr/_bin_size)) += 2;
        if(dr < _r_cut){
    ...
    if (_measure_gofr) {
      ...
      double delta_V = 4.*M_PI/3.*(pow((i*_bin_size+_bin_size),3) - pow(i*_bin_size,3));
      _measurement(_index_gofr+i) /= double(_npart*_rho*delta_V);
      }
    }
  }
  // POFV //////////////////////////////////////////////////////////////////////
...
void System :: averages(int blk){
  ...
  if(_measure_cv) _average(_index_cv) = _beta*_beta*_npart*(_average(_index_cv) - _average(_index_tenergy)*_average(_index_tenergy));
  _global_av  += _average;
