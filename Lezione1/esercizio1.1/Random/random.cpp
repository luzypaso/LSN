...
double Random :: Lorentz(double gamma, double mu) {
   return mu + gamma*tan(M_PI*(Rannyu()-1./2.));
}
...
void Random :: setAll(Random& rnd, int rank) {

   int seed[4];
   int p1, p2;
   ifstream Primes("Primes");
   for (int i=0; i<=rank; ++i) {         //per ogni valore di rank diverso i valori di p1 e p2 sono diversi
      if (Primes.is_open()) {
         Primes >> p1 >> p2;
      } else cerr << "PROBLEM: Unable to open Primes"<<endl;
   }
