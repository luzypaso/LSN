...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    double p = 1. / (1.0 + exp(-2.0*_beta*(_J*(_particle(this->pbc(i-1)).getspin()+_particle(this->pbc(i+1)).getspin()) + _H)));
...
void System :: measure(){ // Measure properties
  ...
  _measurement(_index_chi) = magnetization*magnetization*_beta*double(_npart);
  } //DEVO MOLTIPLICARE PER _npart PERCHE magnetization E' GIA' DIVISO PER _npart
    //E QUI LO STO ELEVANDO AL QUADRATO
...
void System :: averages(int blk){
  ...
  if (_measure_cv) _average(_index_cv) = _beta*_beta*(_average(_index_cv) - _average(_index_tenergy)*_average(_index_tenergy))*double(_npart);
  _global_av  += _average;
