...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    vec sumv(_ndim);                         //VECTOR COUNTING THE # OF PARTICLES IN EACH DIRECTION
    sumv.zeros();
    for (int i=0; i<_npart; i++){
      int x = int(_rnd.Rannyu(0,3));         //SORTING THE AXIAL DIRECTION
      int y = int(_rnd.Rannyu(0,2))*2-1;     //SORTING 1 OR -1 AS THE VERSE 
      if (x==0) vx(i)=y*sqrt(3*_temp);
      else if (x==1) vy(i)=y*sqrt(3*_temp);
      else if (x==2) vz(i)=y*sqrt(3*_temp);
      sumv(0) += vx(i);
      sumv(1) += vy(i);
      sumv(2) += vz(i);
    }
    for (int idim=0; idim<_ndim; idim++) sumv(idim) = sumv(idim)/double(_npart);      //NORMALIZING THE VECTOR
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ofstream coutp("../OUTPUT/potential_energy" + to_string(_nsteps*_nblocks) + ".dat",ios::app);
        ...
        ofstream coutte("../OUTPUT/temperature" + to_string(_nsteps*_nblocks) + ".dat",ios::app);
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(3*_temp)/(double)_n_bins_v; // TO BE FIXED IN EXERCISE 4
        _measure_pofv = true;
...
// Write final configurations as .xyz files in the directory ..//
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_pofv) {
    double v_temp;
    for (int i=0; i<_npart; i++) {
      v_temp = sqrt(dot(_particle(i).getvelocity(), _particle(i).getvelocity()));
      _measurement(_index_pofv + int(v_temp/_bin_size_v)) += 1;
    }
    for (int i=0; i<_n_bins_v; i++) {
      _measurement(_index_pofv+i) /= double(_npart);
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
...
void System :: averages(int blk){
  ...
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
    coutf.open("../OUTPUT/potential_energy" + to_string(_nsteps*_nblocks) + ".dat",ios::app);
  ...
  // TEMPERATURE ///////////////////////////////////////////////////////////////
  if (_measure_temp){
    coutf.open("../OUTPUT/temperature" + to_string(_nsteps*_nblocks) + ".dat",ios::app);
