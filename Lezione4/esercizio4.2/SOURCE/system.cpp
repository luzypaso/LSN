...
void System :: averages(int blk){
  ...
  if (_measure_pofv){
    if (blk == _nblocks) {
      //HO TOLTO LA MODALITA APP PER EVITARE CHE RIFACENDO LA SIMULAZIONE SCRIVA ALTRI DATI OLTRE A QUELLI PRECEDENTI
      //IL PROBLEMA NON SI PONE NEGLI ALTRI ESERCIZI PERCHE I FILE OUTPUT VENGONO SOVRASCRITTI NELLA FUNZIONE INITIALIZE     
      coutf.open("../OUTPUT/pofv" + to_string(_nsteps) + ".dat");   
      for (int i=0; i<_n_bins_v; i++) {
